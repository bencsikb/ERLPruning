""" Based on: https://github.com/TranQuoc.
Implementation of a Transformer encoder Layer with the following input/output sizes: input_ch -> internal_ch//2 -> internal_ch
"""
...
class EncoderLayer(nn.Module):
    def __init__(self, embedding_dim, num_heads, ff_dim=2048, dropout=0.1):
        ...
        self.feed_forward = nn.Sequential(
            nn.Linear(embedding_dim, ff_dim//2),
            nn.ReLU(),
    ...
    def forward(self, x, mask=None):
        print(f"x in EncoderLayer forward before norm: {x.shape}")
        x2 = self.norm1(x)
        print(f"x2 in EncoderLayer forward after norm: {x2.shape}")
        # Add and Muti-head attention
        x = x + self.dropout1(self.self_attention(x2, x2, x2, mask))
        print(f"x in EncoderLayer forward after mhead attention: {x.shape}")
        x2 = self.norm2(x)
        print(f"x2 in EncoderLayer forward after mhead and noprm: {x2.shape}")
...
class Transformer(nn.Module):
    def __init__(self, source_vocab_size, target_vocab_size, source_max_seq_len, target_max_seq_len, embedding_dim,
    ...
    def make_target_mask(self, target_ids):
        ...
        subsequent_mask = (1 - torch.triu(torch.ones((1, len_target, len_target), device=target_ids.device),
                                          diagonal=1)).bool()
...
class Transformer(nn.Module):
    def __init__(self, nhead, dim_model, out_size):
        super(Transformer, self).__init__()
        self.encoder = EncoderLayer(dim_model, nhead, ff_dim=32, dropout=0.1)

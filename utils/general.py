...
def build_targets(p, targets, model):
    ...
    for i, jj in enumerate(model.module.yolo_layers if multi_gpu else model.yolo_layers):
        ...
        anchors = model.module.module_list[jj][0].anchor_vec if multi_gpu else model.module_list[jj][0].anchor_vec
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        #print("boxes in nonmaxsup ", boxes.shape)
        #print("scores in nonmaxsup ", scores.shape)
        i = torchvision.ops.boxes.nms(boxes[:max_det,:], scores[:max_det], iou_thres)
...
def generate_gaussian_probs(mu, sigma, nmb,):
    ...
    while (min(x) < 0):
        x = np.random.normal(mu, sigma, nmb+4)
        y = (2. * np.pi * sigma ** 2.) ** -.5 * np.exp(
            -.5 * (x - mu) ** 2. / sigma ** 2.)  # evaluate the probability density at each point
        x, y = x[np.argsort(y)], np.sort(y)  # sort according to the probability density
        x, y = x.tolist(), y.tolist()
        x.extend(y)
        x = [int(100 - abs(i) * 100) for i in x]
        print(x)
        ...
        tmin = 10
        tmax = 1000
        probs = [int(((m - rmin) / (rmax - rmin)) * (tmax - tmin) + tmin) for m in probs]
        print(probs)

    return probs


def sort_gaussian_probs(x, nmb, layer_index, network_size):

    div = float("{:.1f}".format(network_size/nmb))
    print(div)

    max_i = int(layer_index / div)
    gauss_max_i, gauss_max_val = x.index(max(x)), max(x)
    #maxids = [x.index(i) for i in x if i==gauss_max_val]
    #print(maxids)
    #print(gauss_max_i, x)
    #x[gauss_max_i] -= 1
    #gauss_max_i = x.index(max(x))
    #gauss_max_i = max(maxids)
    print(gauss_max_i, x)
    beg_i = gauss_max_i - max_i
    end_i = beg_i + nmb
    probs = x[beg_i:end_i]
    #with open("sandbox/gaussian.txt", 'a+') as f:
    #   f.write(str(layer_index) + str(probs) + str(len(probs)) + "\n")
    #print(layer_index, probs, len(probs))

    # scale values from 1 to 100
    print(len(x), max_i, gauss_max_i, beg_i, end_i)
    print(probs)
    """
    rmin = min(probs)
    rmax = max(probs)

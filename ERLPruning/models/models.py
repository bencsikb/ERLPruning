    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            if ONNX_EXPORT:  # explicitly state size, avoid scale_factor
                ...
                upsample = nn.Upsample(size=tuple(int(x * g) for x in img_size))
                modules.add_module("Upsample", upsample)
            else:
                #modules = nn.Upsample(scale_factor=mdef['stride'])
                upsample = nn.Upsample(scale_factor=mdef['stride'])
                modules.add_module("Upsample", upsample)
            ...
            wff = WeightedFeatureFusion(layers=layers, weight='weights_type' in mdef)
            modules.add_module("WeightedFeatureFusion", wff)
            ...
            yolo = YOLOLayer(anchors=mdef['anchors'][mdef['mask']],  # anchor list
                                nc=mdef['classes'],  # number of classes
                                img_size=img_size,  # (416, 416)
                                yolo_index=yolo_index,  # 0, 1, 2...
                                layers=layers,  # output layers
                                stride=stride[yolo_index])
            modules.add_module("YOLOLayer", yolo)
...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p, out):
        ...
        if self.training:
            #print("Line 258, self.training true")
            return p

        elif ONNX_EXPORT:
            #print("Line 258, onnx export true")
...
class Darknet(nn.Module):
    ...
    def forward_once(self, x, augment=False, verbose=False):
        ...
        for i, module in enumerate(self.module_list):
            name = module[0].__class__.__name__
            if name in ['WeightedFeatureFusion', 'FeatureConcat', 'FeatureConcat2', 'FeatureConcat3', 'FeatureConcat_l']:  # sum, concat
                ...
                #print("i ", i, "  ", x.shape)
                x = module[0](x, out)  # WeightedFeatureFusion(), FeatureConcat()
            elif name == 'YOLOLayer':
                yolo_out.append(module[0](x, out))
...
def get_yolo_layers(model):
        return [i for i, m in enumerate(model.module_list) if m[0].__class__.__name__ == 'YOLOLayer']  # [89, 101, 113]
